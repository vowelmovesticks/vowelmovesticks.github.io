// quick fix
/*const fs = require("fs")
var homedir = "/home/max/stickylingo"
...
class stickylingo {
  ...
  gl(text, langa, langb="en", tlit=false, marksourceword=false) {
      ...
      text = stickylingo.insertbreaks(text, langa)
      var a = stickylingo.split(text, langa)
      var out = ""
      for (var w of a) {
  	// w is non-word
  	if (!w.match(/\p{L}/u)) {
  	    out += w
     	 		...
     	 		insert = this._mocktranslate(insert, langb)
     	             ...
     	             insert = insert.replaceAll(/^[^\p{L}\p{M}]+/gu, "")
     	             insert = insert.replaceAll(/[^\p{L}\p{M}]+$/gu, "")	    
     	             if (tlit == true) {
     	 	        w_orig = translit(w_orig, { translitLatin: false })
     	 	    }
     	 	    if (marksourceword) {
     	 		out += "<<" + w_orig + ">>"
     	 	    } else {
  ...
  glhtml(text, langa, langb="en", tlit=false) {
    return stickylingo.gltohtml(this.gl(text, langa, langb, tlit))
  }
  static gltohtml(s) {
      ...
      s = s.replaceAll(/\]\] /g, '<\/span>&nbsp; ')
      s = s.replaceAll(/\]\]([^\p{L}\p{M}\s]+) ?/gu, '<\/span>$1&nbsp; ')
      s = s.replaceAll(/\]\]/g, '<\/span>&nbsp; ') 
      s = '<span style="font-weight: bold">' + s + '</span>'
      return s
  }
  _mocktranslate(en, lang) {
      var wrds = stickylingo.words(en)
  ...
  adddict(newdict) {
    this.dict = stickylingo.merge(this.dict, newdict)
  }
  static merge(dto, dfrom) {
      ...
      for (var lang of Object.keys(dfrom)) {
         	...
         	if (lang == "en") {
         	    out["en"] = stickylingo.merge(out["en"], dfrom["en"])
         	}
                 else { 
